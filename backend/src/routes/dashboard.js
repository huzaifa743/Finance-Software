...
router.get('/', authenticate, (req, res) => {
  ...
  // Treat deposits created from sale records and receivable recoveries as direct-to-bank receipts (do not reduce cash-in-hand)
  const bankDeposits = db.prepare(`
    SELECT COALESCE(SUM(amount), 0) as t
    FROM bank_transactions
    WHERE type IN ('deposit', 'transfer_in')
      AND (reference IS NULL OR reference NOT LIKE 'sale-%')
      AND (description IS NULL OR description NOT LIKE 'Receivable recovery #%')
  `).get();
